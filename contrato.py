...
class Contrato(object):
    ...
    def avanca(self):
        if self.__tipo == 'NOVO':
            self.__tipo = 'ANDAMENTO'
        elif self.__tipo == 'ANDAMENTO':
            self.__tipo = 'ACERTADO'
        elif self.__tipo == 'ACERTADO':
            self.__tipo = 'CONCLUIDO'

    def salva_estado(self):
        return Estado(Contrato(data=self.__data, cliente=self.__cliente, tipo=self.__tipo))
...
if __name__ == '__main__':
    ...
    contrato = Contrato(data=date.today, cliente='Simba Lima Pimentel')
    ...
    print('Tipo de contrato restaurado: {}'.format(contrato.tipo))